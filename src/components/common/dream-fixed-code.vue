<template>
  ...
  <div
    :id="id"
    :class="['vue-puzzle-vcode', { show_: show, disabled_: disabled }]"
  ...
  >
    <div class="vue-auth-box_" @mousedown.stop @touchstart.stop>
      <div class="auth-body_" :style="`height: ${canvasHeight}px`">
        ...
        <div :class="['info-box_', { show: infoBoxShow }, { fail: infoBoxFail }]">{{ infoText }}</div>
        <div
...
<script>
import resetSvg from "@/assets/img/common/reset.png";
import { getCaptchaImage, validCaptcha } from '@/api'
...
export default {
  ...
  computed: {
    successText() {
      return this.$t("common.components.vCode.text.success");
    },
    failText() {
      return this.$t("common.components.vCode.text.fail");
    },
    sliderText() {
      return this.$t("common.components.vCode.text.slider");
    },
  ...
  methods: {
    ...
    init(withCanvas) {
      ...
      img.onload = () => {
        ...
        ctx2.drawImage(smallImg, 2, smallImgY + 3, 47, 155);
        // 清理
        ctx.restore();
        ctx.clearRect(0, 0, this.canvasWidth, this.canvasHeight);
        // 画缺口
      ...
      if (!withCanvas && this.imgs && this.imgs.length) {
        ...
        this.getImgData().then(res => {
          smallImg.src = `data:image/png;base64,${res.data.smallImage}`;
          smallImg.width = 47;
          smallImg.height = 155;
          img.src = `data:image/png;base64,${res.data.bigImage}`;
          smallImgY = res.data.location_y * 1 - 2;
          this.checkRandom = res.data.random;
        });
      }
    },
    // 工具 - 范围随机数
    ...
    // 用canvas随机生成图片
    makeImgWithCanvas() {
      // this.$axios.get("/user/api/user/captchaImage", {}).then(res => {
    ...
    submit() {
      ...
      //     3
      // );
      const x = this.styleWidth - this.sliderSize - 3;
      validCaptcha({
        position_x: Math.round(x),
        random: this.checkRandom
      }).then(res => {
          if (res.succ) {
            // 成功
